#!/usr/bin/env node
// Build the single npm package for j18n.
//
// Layout:
//   - 1 package (@j18n/cli) — bundles all 6 platform binaries under
//     `bin/<key>/j18n[.exe]` and ships a tiny stub at `bin/j18n.js` that
...
// every install pulls all six binaries instead of one, but the install path is
...
//     --out npm-dist
//
// Reads tarballs/zips named like the release produces:
//   j18n-cli-aarch64-apple-darwin.tar.xz
//   j18n-cli-x86_64-pc-windows-msvc.zip
//   etc.
...
const REPO_URL = 'https://github.com/Skiley/j18n';

interface Platform {
	/** Matches `${process.platform}-${process.arch}` at runtime in the stub. */
	readonly key: string;
	/** Filename of the release artifact (relative to the artifacts dir). */
	readonly archive: string;
	/** Filename of the binary inside the artifact (`j18n` on Unix, `j18n.exe` on Windows). */
	readonly bin: string;
}

const PLATFORMS: readonly Platform[] = [
	{ key: 'darwin-arm64', archive: 'j18n-cli-aarch64-apple-darwin.tar.xz', bin: 'j18n' },
	{ key: 'darwin-x64', archive: 'j18n-cli-x86_64-apple-darwin.tar.xz', bin: 'j18n' },
	{ key: 'linux-arm64', archive: 'j18n-cli-aarch64-unknown-linux-musl.tar.xz', bin: 'j18n' },
	{ key: 'linux-x64', archive: 'j18n-cli-x86_64-unknown-linux-musl.tar.xz', bin: 'j18n' },
	{ key: 'win32-arm64', archive: 'j18n-cli-aarch64-pc-windows-msvc.zip', bin: 'j18n.exe' },
	{ key: 'win32-x64', archive: 'j18n-cli-x86_64-pc-windows-msvc.zip', bin: 'j18n.exe' },
];
...
function extractBinary(archivePath: string, binName: string, destDir: string): void {
	...
	if (archivePath.endsWith('.tar.xz')) {
		// Release tarballs contain a top-level dir like j18n-cli-<target>/j18n.
		...
		// Zip layout: <zipname>/j18n.exe — extract everything then move
...
function writePackage({ outDir, version, artifactsDir }: WritePackageOptions): WritePackageResult {
	...
	const pkgJson = {
		...
		version,
		description: 'j18n CLI — LLM-powered syncing of translated JSON locale files',
		repository: REPO_URL,
		homepage: REPO_URL,
		license: 'MIT',
		bin: { j18n: 'bin/j18n.js' },
		files: ['bin'],
		engines: { node: '>=16' },
	};
...
`;
	writeFileSync(join(binDir, 'j18n.js'), stub, { mode: 0o755 });
